...
!(function (factory) {
  ...
  function TouchSwipe(element, options) {
      ...
      if (!(getTouchInProgress() || $(jqEvent.target).closest(options.excludedElements, $element).length > 0)) {
        ...
        if (!event.pointerType || "mouse" != event.pointerType || 0 != options.fallbackToMouseEvents) {
          ...
          return (
            ...
            !1 === ret
              ? (triggerHandler(event, (phase = PHASE_CANCEL)), ret)
              : (options.hold &&
    ...
    function touchMove(jqEvent) {
      ...
      if (phase !== PHASE_END && phase !== PHASE_CANCEL && !inMultiFingerRelease()) {
        var ret,
          touches = event.touches,
          evt,
          currentFinger = updateFingerData(touches ? touches[0] : event);
        ...
        } else triggerHandler(event, (phase = PHASE_CANCEL));
        !1 === ret && triggerHandler(event, (phase = PHASE_CANCEL));
      }
    }
    ...
    function touchLeave(jqEvent) {
      var event = jqEvent.originalEvent ? jqEvent.originalEvent : jqEvent;
      options.triggerOnTouchLeave && triggerHandler(event, (phase = getNextPhase(PHASE_END)));
    }
    ...
    function triggerHandler(event, phase) {
      var ret,
        touches = event.touches;
      return (didSwipe() || hasSwipes()) && (ret = triggerHandlerForGesture(event, phase, SWIPE)), (didPinch() || hasPinches()) && !1 !== ret && (ret = triggerHandlerForGesture(event, phase, PINCH)), didDoubleTap() && !1 !== ret ? (ret = triggerHandlerForGesture(event, phase, DOUBLE_TAP)) : didLongTap() && !1 !== ret ? (ret = triggerHandlerForGesture(event, phase, LONG_TAP)) : didTap() && !1 !== ret && (ret = triggerHandlerForGesture(event, phase, TAP)), phase === PHASE_CANCEL && touchCancel(event), phase === PHASE_END && ((touches && touches.length) || touchCancel(event)), ret;
    }
    function triggerHandlerForGesture(event, phase, gesture) {
      var ret;
      if (gesture == SWIPE) {
        if (($element.trigger("swipeStatus", [phase, direction || null, distance || 0, duration || 0, fingerCount, fingerData, currentDirection]), options.swipeStatus && !1 === (ret = options.swipeStatus.call($element, event, phase, direction || null, distance || 0, duration || 0, fingerCount, fingerData, currentDirection)))) return !1;
        if (phase == PHASE_END && validateSwipe()) {
          if ((clearTimeout(singleTapTimeout), clearTimeout(holdTimeout), $element.trigger("swipe", [direction, distance, duration, fingerCount, fingerData, currentDirection]), options.swipe && !1 === (ret = options.swipe.call($element, event, direction, distance, duration, fingerCount, fingerData, currentDirection)))) return !1;
          switch (direction) {
            case LEFT:
      ...
      if (gesture == PINCH) {
        if (($element.trigger("pinchStatus", [phase, pinchDirection || null, pinchDistance || 0, duration || 0, fingerCount, pinchZoom, fingerData]), options.pinchStatus && !1 === (ret = options.pinchStatus.call($element, event, phase, pinchDirection || null, pinchDistance || 0, duration || 0, fingerCount, pinchZoom, fingerData)))) return !1;
    ...
    function inMultiFingerRelease() {
      var withinThreshold = !1,
        diff;
      previousTouchEndTime && getTimeStamp() - previousTouchEndTime <= options.fingerReleaseThreshold && (withinThreshold = !0);
    ...
    function setTouchInProgress(val) {
      $element && (!0 === val ? ($element.bind(MOVE_EV, touchMove), $element.bind(END_EV, touchEnd), LEAVE_EV && $element.bind(LEAVE_EV, touchLeave)) : ($element.unbind(MOVE_EV, touchMove, !1), $element.unbind(END_EV, touchEnd, !1), LEAVE_EV && $element.unbind(LEAVE_EV, touchLeave, !1)), $element.data(PLUGIN_NS + "_intouch", !0 === val));
    }
    ...
    function getbounds(el) {
      var offset = (el = $(el)).offset(),
        bounds;
      return { left: offset.left, right: offset.left + el.outerWidth(), top: offset.top, bottom: offset.top + el.outerHeight() };
    }
